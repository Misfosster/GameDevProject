...
public class SphereController : MonoBehaviour
{
    ...
    void RotateCamera()
    {
        if (cameraTransform != null)
        {
            ...
            currentAngleVertical = Mathf.Clamp(currentAngleVertical, -90f, 90f);

            Quaternion rotation = Quaternion.Euler(currentAngleVertical, currentAngleHorizontal, 0);

            // Multiply the camera offset by the scale factor
    ...
    void CheckGround()
    {
        ...
        RaycastHit hit;
        float distance = 1.7f; // Adjust this value as needed
    ...
    void WallCheck()
{
    ...
    // Use the sphere's position as the starting point for the ray
    if (Physics.Raycast(transform.position, cameraTransform.forward, out hit, wallCheckDistance, obstacleLayer))
    {
        // If an obstacle is detected, adjust the camera's position to avoid it
        // Use a dynamic offset based on the hit normal to prevent camera intersection
        cameraTransform.position = hit.point + hit.normal * 0.2f; // Adjust the offset based on the hit normal
    }
}
